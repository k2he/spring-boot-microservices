...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
  ...
  protected String determineTargetUrl(HttpServletRequest request, HttpServletResponse response,
    ...
    Optional<String> redirectUri =
        Optional.of(httpCookieOAuth2AuthorizationRequestRepository.getREDIRECT_URI_PARAM());
    ...
    JwtAuthenticationToken jwtToken =
        new JwtAuthenticationToken(token, (UserPrincipal) authentication.getPrincipal());
    ObjectMapper mapper = new ObjectMapper();
    String jsonInString = mapper.writeValueAsString(jwtToken);

    return UriComponentsBuilder.fromUriString(targetUrl).queryParam("OAuth2Response", jsonInString)
        .build().toUriString();
  }
